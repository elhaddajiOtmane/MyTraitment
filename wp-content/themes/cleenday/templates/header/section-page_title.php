...
if (!class_exists('Cleenday_Get_Page_Title')) {
    /**
     *  Page Title Area
     *
    ...
    class Cleenday_Get_Page_Title
    {
        ...
        public function get_page_header_attributes()
        {
            ...
            // Parallax
            $parallax_enabled = Cleenday::get_mb_option('page_title_parallax', 'mb_page_title_switch', 'on');
            if ($parallax_enabled) {
                wp_enqueue_script('jquery-paroller', get_template_directory_uri() . '/js/jquery.paroller.min.js');
                $parallax_class = ' page_title_parallax';
                $parallax_speed = apply_filters('cleenday/page_title/parallax_speed', Cleenday::get_mb_option('page_title_parallax_speed', 'mb_page_title_switch', 'on'));
        ...
        public function get_page_header_classes()
        {
            if (
                ...
                if ($this->RWMB_is_active()) {
                    ...
                    $page_title_align = Cleenday::get_option($post_type . '_single_title_align');
                    $breadcrumbs_align = Cleenday::get_option($post_type . '_single_breadcrumbs_align');
                    $breadcrumbs_block = Cleenday::get_option($post_type . '_single_breadcrumbs_block_switch');
                }
            } else {
                $page_title_align = Cleenday::get_mb_option('page_title_align', 'mb_page_title_switch', 'on');
                $breadcrumbs_align = Cleenday::get_mb_option('page_title_breadcrumbs_align', 'mb_page_title_switch', 'on');
                $breadcrumbs_block = Cleenday::get_mb_option('page_title_breadcrumbs_block_switch', 'mb_page_title_switch', 'on');
            }
        ...
        {
            $bg_enabled = Cleenday::get_option($this->post_type . '_' . $this->post_query . '__page_title_bg_switch')
                ?? Cleenday::get_option('page_title_bg_switch');

            $min_height = Cleenday::get_option($this->post_type . '_' . $this->post_query . '__page_title_height')['height'] ?? 'px';
            $min_height = 'px' !== $min_height ? $min_height : Cleenday::get_option('page_title_height')['height'];

            $bg_color = Cleenday::get_option($this->post_type . '_' . $this->post_query . '__page_title_bg_image')['background-color'] ?? '';
            $bg_color = $bg_color ?: Cleenday::get_option('page_title_bg_image')['background-color'];

            $paddings = Cleenday::get_option($this->post_type . '_' . $this->post_query . '__page_title_padding') ?: [];
            if (!isset($paddings['padding-top']) || '' === $paddings['padding-top']) {
                $paddings['padding-top'] = Cleenday::get_option('page_title_padding')['padding-top'] ?? '';
            }
            if (!isset($paddings['padding-bottom']) || '' === $paddings['padding-bottom']) {
                $paddings['padding-bottom'] = Cleenday::get_option('page_title_padding')['padding-bottom'] ?? '';
            }

            $margin_bottom = Cleenday::get_option($this->post_type . '_' . $this->post_query . '__page_title_margin')['margin-bottom'] ?? '';
            $margin_bottom = '' !== $margin_bottom ? $margin_bottom : Cleenday::get_option('page_title_margin')['margin-bottom'];
        ...
        {
            $bg_enabled = Cleenday::get_option('page_title_bg_switch');
            $bg_color = Cleenday::get_option('page_title_bg_image')['background-color'];
            $min_height = Cleenday::get_option('page_title_height')['height'];
            $paddings = Cleenday::get_option('page_title_padding');
            $margin_bottom = Cleenday::get_option('page_title_margin')['margin-bottom'] ?? '';
        ...
        {
            if (is_404()) {
                return Cleenday::bg_render('404_page__page_title') ?: Cleenday::bg_render('page_title');
            }

            if (
                function_exists('is_woocommerce') && is_woocommerce()
                && !empty($bg_shop = Cleenday::bg_render($this->post_type . '_' . $this->post_query . '_page_title'))
            ) {
                return !is_product()
                    ? $bg_shop
                    : Cleenday::bg_render('shop_single_page_title');
            }

            if (
                $this->post_type
                && !empty($bg_cpt = Cleenday::bg_render($this->post_type . '_' . $this->post_query . '__page_title'))
            ) {
                return $bg_cpt;
            }

            return Cleenday::bg_render('page_title', 'mb_page_title_switch', 'on');
        }
        ...
        public function get_title_text()
        {
            if (is_home() && is_front_page()) {
                ...
                $title = esc_html(get_queried_object()->post_title) ?? '';
                $title = apply_filters('cleenday/page_title/blog_archive', $title);
            } elseif (is_category()) {
                $title = single_cat_title('', false);
                $title = apply_filters('cleenday/page_title/category_title', $title);
            } elseif (is_tag()) {
                $title = single_term_title('', false) . esc_html__(' Tag', 'cleenday');
	            $title = apply_filters('cleenday/page_title/tag_title', $title);
            } elseif (is_date()) {
                $title = get_the_time('F Y');
	            $title = apply_filters('cleenday/page_title/date_title', $title);
            } elseif (is_author()) {
                $title = esc_html__('Author:', 'cleenday') . ' ' . get_the_author();
	            $title = apply_filters('cleenday/page_title/author_title', $title);
            } elseif (is_search()) {
                $title = esc_html__('Search', 'cleenday');
	            $title = apply_filters('cleenday/page_title/search_title', $title);
            } elseif (is_404()) {
                $this->page_title_tag = 'h1';
                $title = Cleenday::get_option('404_custom_title_switch')
                    ? esc_html(Cleenday::get_option('404_page_title_text'))
                    : esc_html__('Error Page', 'cleenday');
	            $title = apply_filters('cleenday/page_title/404_title', $title);
            } elseif (is_singular('portfolio')) {
                $title = Cleenday::get_option('portfolio_title_conditional')
                    ? esc_html__('Portfolio', 'cleenday')
                    : esc_html(get_the_title());
                $title = apply_filters('cleenday/page_title/portfolio_single', $title);
            } elseif (is_singular('team')) {
                $title = Cleenday::get_option('team_title_conditional')
                    ? esc_html__('Team', 'cleenday')
                    : esc_html(get_the_title());
                $title = apply_filters('cleenday/page_title/team_single', $title);
            } elseif (function_exists('is_product') && is_product()) {
                $title = Cleenday::get_option('shop_title_conditional')
                    ? esc_html__('Shop', 'cleenday')
                    : esc_html(get_the_title());
                $title = apply_filters('cleenday/page_title/shop_single', $title);
            } elseif (is_archive()) {
                if (function_exists('is_shop')
                    && (is_shop() || is_product_category() || is_product_tag())
                ) {
                    $title = esc_html__('Shop', 'cleenday');
	                $title = apply_filters('cleenday/page_title/shop', $title);
                } elseif (is_tax(['portfolio_tag', 'portfolio-category'])) {
                    $title = esc_html__('Portfolio', 'cleenday');
                    $title = apply_filters('cleenday/page_title/portfolio_archive', $title);
                } else {
                    $title = esc_html__('Archive', 'cleenday');
	                $title = apply_filters('cleenday/page_title/archive', $title);
                }
            } else {
                global $post;
                if (!empty($post)) {
                    ...
                    if ('post' == get_post_type($post)) {
                        ...
                        $title = Cleenday::get_option('blog_title_conditional')
                            ? esc_html__('Blog Post', 'cleenday')
                            : esc_html(get_the_title($post->ID));
                        $title = apply_filters('cleenday/page_title/blog_single', $title);
                    } else {
                        $title = esc_html(get_the_title($post->ID));
	                    $title = apply_filters('cleenday/page_title/the_title', $title);
                    }
                } else {
                    $title = esc_html__('No Posts', 'cleenday');
	                $title = apply_filters('cleenday/page_title/no_posts', $title);
                }
            }
            ...
            if (
                ...
                $title = apply_filters('cleenday/page_title/custom_text', $title);
            }
        ...
        {
            $user_tag = Cleenday::get_mb_option('page_title_tag', 'mb_page_title_switch', 'on');
        ...
        {
            $pt_font = Cleenday::get_mb_option('page_title_font', 'mb_page_title_switch', 'on');
        ...
        public function is_breadcrumbs_enabled()
        {
            if (
                ...
                return 'on' == $this->mb_page_title_switch
                    ...
                    : Cleenday::get_option('blog_single_page_title_breadcrumbs_switch');
            }

            return Cleenday::get_mb_option('page_title_breadcrumbs_switch', 'mb_page_title_switch', 'on');
        }

        public function get_breadcrumbs()
        {
            $breadcrumbs_font = Cleenday::get_mb_option('page_title_breadcrumbs_font', 'mb_page_title_switch', 'on');
